...
class AdminHelloDelete extends Controller
{
    ...
    /**
     * __DIR__에서 AdminHello.json 파일을 읽어오는 메소드
     */
    private function loadJsonFromCurrentPath()
    {
        try {
            $jsonFilePath = __DIR__ . DIRECTORY_SEPARATOR . 'AdminHello.json';
    ...
    {
        // 데이터베이스에서 데이터 조회
        $tableName = $this->jsonData['table']['name'] ?? 'admin_hellos';
    ...
    private function logDeletion($data)
    {
        ...
        Log::channel($channel)->$level('Hello deleted', $logData);
    }
